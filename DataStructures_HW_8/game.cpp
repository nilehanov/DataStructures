...
{
    std::cout<<p.getName()<<" stays in "<<p.getLocation()->getName()<<"\n";
...
City* MOVES_RANDOMLY(const Person& p, const Graph& city_graph)
{
    ...
    {
        std::cout<<p.getName()<<" moves from "<<p.getLocation()->getName()<<" to "<<neighbors[r-1]->getName()<<"\n";
    }
...
City* QUERY_USER(const Person& p, const Graph& city_graph) {
    ...
    if (i == 0)
    {
        std::cout<<p.getName()<<" stays in "<<p.getLocation()->getName()<<std::endl;
        return p.getLocation();
    }
    else
    {
        std::cout<<p.getName()<<" moves from "<<p.getLocation()->getName()<<" to "<<neighbors[i-1]->getName()<<std::endl;
    }
